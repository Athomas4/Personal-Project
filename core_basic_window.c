...
int main(void)
{
    ...
    while (!WindowShouldClose())  // Detect window close button or ESC key
    {
        ...
        // Function to check if two helper blocks collide
bool CheckCollisionBetweenHelpers(Helper helper1, Helper helper2) {
    // Check for overlap in both the X and Y dimensions
    return (helper1.position.x < helper2.position.x + 20 &&
            helper1.position.x + 20 > helper2.position.x &&
            helper1.position.y < helper2.position.y + 20 &&
            helper1.position.y + 20 > helper2.position.y);
}

        // Function to adjust the position of a helper block if it collides with another one
void ResolveHelperCollision(Helper* helper1, Helper* helper2) {
    // Simple collision resolution: move one helper out of the way
    if (CheckCollisionBetweenHelpers(*helper1, *helper2)) {
        // For simplicity, move helper1 to the right of helper2 (or vice versa)
        helper1->position.x += 30; // You can adjust this value to your preference
    }
}

        // Make helpers go collect boxes and check for collisions between them
for (int i = 0; i < helperCount; i++) {
    if (helpers[i].active) {
        ...
        // Check for collisions with other helpers
        for (int j = 0; j < helperCount; j++) {
            if (i != j && helpers[i].active && helpers[j].active) {
                ResolveHelperCollision(&helpers[i], &helpers[j]);
            }
        }
    }
}
